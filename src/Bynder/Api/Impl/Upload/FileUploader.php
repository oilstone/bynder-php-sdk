...
class FileUploader
{
	 ...
	 * @throws Exception
	 */
	private function finalizeUploadAdditionalAsync($mediaId, $uploadRequestInfo, $chunkNumber)
	{
		$s3Filename = sprintf("%s/p%d", $uploadRequestInfo['s3_filename'], $chunkNumber);
		...
		return $this->requestHandler->sendRequestAsync(
			'POST',
			sprintf('api/v4/media/%s/save/additional/%s/', $mediaId, $uploadRequestInfo['s3file']['uploadid']),
			['form_params' => $data]
		);
	}
